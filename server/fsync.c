...
int fsync_check_support(void)
{
    ...
    /* Probe only when the user actually asked for fsync: the probe itself is a
     * real futex_waitv() call and OHOS' app sandbox answers forbidden syscalls
     * with SIGSYS, which would kill wineserver before it ever prints anything. */
    if (!getenv( "WINEFSYNC" ) || !atoi( getenv( "WINEFSYNC" ) )) return 0;
    syscall( __NR_futex_waitv, 0, 0, 0, 0, 0 );
    return errno != ENOSYS && errno != EPERM;
#else
    /* OHOS: futex_waitv() is not permitted inside the app sandbox (SIGSYS). */
    return 0;
#endif
}
