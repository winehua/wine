 ...
 */
/* WineHua: force Chromium/CEF child processes onto the software rendering path.
 * CEF's GPU stack (ANGLE over D3D11/Vulkan/GL) cannot initialize in this runtime, which
 * leaves steamwebhelper's login window unpainted.  Only active when the environment asks
 * for it, so ordinary applications are unaffected. */
static WCHAR *winehua_cef_append_software_switches( const WCHAR *cmdline )
{
    static const WCHAR needle[] = L"steamwebhelper";
    /* WineHua TEMP-DIAG/产品化: 每个门禁独立, 互不干扰。
     *   WINEHUA_CEF_FORCE_SOFTWARE=1 -> --disable-gpu --disable-gpu-compositing
     *     (CEF 的 ANGLE/D3D11 栈在本 runtime 起不来, 必须走软件合成)
     *   WINEHUA_CEF_LOGGING=1        -> --enable-logging=file --log-file=C:\windows\temp\cef-verbose.log --v=1
     *     (把 Chromium 自己的 verbose 日志落到 prefix 里的文件; 实测 stderr 模式在这条
     *      路径上会**分配一个控制台窗口**把日志画在桌面上 —— 既污染界面又抢焦点,
     *      所以产品路径只提供文件模式) */
    static const WCHAR extra_software[] = L" --disable-gpu --disable-gpu-compositing";
    static const WCHAR extra_soft_compositing[] = L" --disable-gpu-compositing";
    static const WCHAR extra_soft_gpu[] = L" --disable-gpu";
    static const WCHAR extra_logging[]  =
        L" --v=1";
    /* P0-GL: 让 CEF/ANGLE 显式选渲染后端。实测 guest Mesa(经 virgl/vtest)只到
     * GL 2.1/ES2 (ES3 context 直接 EGL_BAD_CONFIG), 而 DXVK/Venus 这条 Vulkan
     * 链在本 runtime 是通的 —— 所以 ANGLE 走 vulkan 后端比走 GL 更有希望出画面。
     * WINEHUA_CEF_ANGLE_BACKEND=vulkan|d3d11|gl|swiftshader */
    static const WCHAR extra_angle_vulkan[] =
        L" --use-gl=angle --use-angle=vulkan";
    static const WCHAR extra_angle_d3d11[] =
        L" --use-gl=angle --use-angle=d3d11";
    /* 定向 Chromium VLOG: --vmodule=<spec> (例如 render_process_host_impl=2,...)。
     * 只加 vmodule, 不动日志通道 (Steam 自己的 --enable-logging=handle 保留)。 */
    static WCHAR extra_vmodule[192];
    WCHAR flag[2];
    WCHAR sw[16];
    WCHAR angle[16];
    const WCHAR *extras[5];
    int n_extras = 0;
    const WCHAR *p;
    SIZE_T len, extra_len = 0;
    WCHAR *ret;
    int i;

    /* WINEHUA_CEF_FORCE_SOFTWARE 的取值决定关掉哪一段 GPU 栈 (2026-09-18 观测轮):
     *   (未设) / 1 / both  → --disable-gpu --disable-gpu-compositing (历史默认)
     *   compositing        → 只关合成器 (保留 GPU 光栅化)
     *   gpu                → 只关 GPU 进程 (保留合成器走软件回退)
     *   0                  → 都不加 (native 对照)
     * 加这两个开关本身也会改变 CEF 行为 (实测 win64 在强制软件下 renderer 会
     * SIGSEGV 反复重启), 所以必须能分开 A/B, 而不是只能全开或全关。 */
    if (GetEnvironmentVariableW( L"WINEHUA_CEF_FORCE_SOFTWARE", sw, 16 ) && sw[0])
    {
        if (sw[0] == '1' || !lstrcmpiW( sw, L"both" ))
            extras[n_extras++] = extra_software;
        else if (!lstrcmpiW( sw, L"compositing" ))
            extras[n_extras++] = extra_soft_compositing;
        else if (!lstrcmpiW( sw, L"gpu" ))
            extras[n_extras++] = extra_soft_gpu;
    }
    if (GetEnvironmentVariableW( L"WINEHUA_CEF_LOGGING", flag, 2 ) == 1 && flag[0] == '1')
        extras[n_extras++] = extra_logging;
    UINT angle_len = GetEnvironmentVariableW( L"WINEHUA_CEF_ANGLE_BACKEND", angle, 16 );
    if (angle_len && angle_len < 16)
    {
        if (!lstrcmpiW( angle, L"vulkan" )) extras[n_extras++] = extra_angle_vulkan;
        else if (!lstrcmpiW( angle, L"d3d11" )) extras[n_extras++] = extra_angle_d3d11;
    }
    {
        WCHAR vmod[160];
        UINT vlen = GetEnvironmentVariableW( L"WINEHUA_CEF_VMODULE", vmod, 160 );
        if (vlen && vlen < 160)
        {
            _snwprintf( extra_vmodule, 192, L" --vmodule=%s", vmod );
            extra_vmodule[191] = 0;
            extras[n_extras++] = extra_vmodule;
        }
    }
    if (!n_extras) return NULL;

    for (p = cmdline; *p; p++)
    {
        const WCHAR *q = needle;
        const WCHAR *s = p;
        while (*q && *s && ((*s | 0x20) == *q)) { q++; s++; }
        if (!*q) break;
    }
    if (!*p) return NULL;

    len = lstrlenW( cmdline );
    for (i = 0; i < n_extras; i++) extra_len += lstrlenW( extras[i] );
    if (!(ret = RtlAllocateHeap( GetProcessHeap(), 0, (len + extra_len + 1) * sizeof(WCHAR) )))
        return NULL;
    memcpy( ret, cmdline, len * sizeof(WCHAR) );
    extra_len = len;
    for (i = 0; i < n_extras; i++)
    {
        SIZE_T n = lstrlenW( extras[i] );
        memcpy( ret + extra_len, extras[i], n * sizeof(WCHAR) );
        extra_len += n;
    }
    ret[extra_len] = 0;
    return ret;
                                                           ...
                                                           const STARTUPINFOW *startup )
{
    ...
    cef_cmdline = winehua_cef_append_software_switches( cmdline );
    RtlInitUnicodeString( &cmdlineW, cef_cmdline ? cef_cmdline : cmdline );
    ...
    {
        if (cef_cmdline) RtlFreeHeap( GetProcessHeap(), 0, cef_cmdline );
