...
static NTSTATUS ohos_main_loop(void *args)
{
...
static NTSTATUS ohos_timer_loop(void *args)
{
    ...
    period = (REFERENCE_TIME)(stream->client_period_frames ? stream->client_period_frames : 1) * 10000000;
    period /= max(stream->fmt->Format.nSamplesPerSec, 1u);
    period = max(period, (REFERENCE_TIME)10000);
    delay.QuadPart = -period;
    ...
    while (!stream->please_quit)
    {
        ...
        if (stream->please_quit) break;
        if (stream->event && stream->started) NtSetEvent(stream->event, NULL);
    }
    ohos_unlock(stream);
    return STATUS_SUCCESS;
}
...
static const char audio_unixlib_abi_marker[] __attribute__((used)) =
    "WineHua audio unixlib ABI slots v1";

const unixlib_entry_t __wine_unix_call_funcs[] =
{
    [process_attach] = ohos_process_attach,
    [process_detach] = ohos_process_detach,
    [main_loop] = ohos_main_loop,
    [get_endpoint_ids] = ohos_get_endpoint_ids,
    [create_stream] = ohos_create_stream,
    [release_stream] = ohos_release_stream,
    [start] = ohos_start,
    [stop] = ohos_stop,
    [reset] = ohos_reset,
    [timer_loop] = ohos_timer_loop,
    [get_render_buffer] = ohos_get_render_buffer,
    [release_render_buffer] = ohos_release_render_buffer,
    [get_capture_buffer] = ohos_get_capture_buffer,
    [release_capture_buffer] = ohos_release_capture_buffer,
    [is_format_supported] = ohos_is_format_supported,
    [get_loopback_capture_device] = ohos_not_implemented,
    [get_mix_format] = ohos_get_mix_format,
    [get_device_period] = ohos_get_device_period,
    [get_buffer_size] = ohos_get_buffer_size,
    [get_latency] = ohos_get_latency,
    [get_current_padding] = ohos_get_current_padding,
    [get_next_packet_size] = ohos_get_next_packet_size,
    [get_frequency] = ohos_get_frequency,
    [get_position] = ohos_get_position,
    [set_volumes] = ohos_set_volumes,
    [set_event_handle] = ohos_set_event_handle,
    [set_sample_rate] = ohos_not_implemented,
    [test_connect] = ohos_test_connect,
    [is_started] = ohos_is_started,
    [get_prop_value] = ohos_get_prop_value,
    [midi_get_driver] = ohos_midi_get_driver,
    [midi_init] = ohos_midi_init,
    [midi_release] = ohos_midi_release,
    [midi_out_message] = ohos_midi_out_message,
    [midi_in_message] = ohos_midi_in_message,
    [midi_notify_wait] = ohos_midi_notify_wait,
    [aux_message] = ohos_aux_message,
};
...
static NTSTATUS ohos_wow64_main_loop(void *args)
{
    struct
    {
        PTR32 event;
...
{
    [process_attach] = ohos_wow64_process_attach,
    [process_detach] = ohos_process_detach,
    [main_loop] = ohos_wow64_main_loop,
    [get_endpoint_ids] = ohos_wow64_get_endpoint_ids,
    [create_stream] = ohos_wow64_create_stream,
    [release_stream] = ohos_wow64_release_stream,
    [start] = ohos_start,
    [stop] = ohos_stop,
    [reset] = ohos_reset,
    [timer_loop] = ohos_timer_loop,
    [get_render_buffer] = ohos_wow64_get_render_buffer,
    [release_render_buffer] = ohos_release_render_buffer,
    [get_capture_buffer] = ohos_wow64_get_capture_buffer,
    [release_capture_buffer] = ohos_release_capture_buffer,
    [is_format_supported] = ohos_wow64_is_format_supported,
    [get_loopback_capture_device] = ohos_not_implemented,
    [get_mix_format] = ohos_wow64_get_mix_format,
    [get_device_period] = ohos_wow64_get_device_period,
    [get_buffer_size] = ohos_wow64_get_buffer_size,
    [get_latency] = ohos_wow64_get_latency,
    [get_current_padding] = ohos_wow64_get_current_padding,
    [get_next_packet_size] = ohos_wow64_get_next_packet_size,
    [get_frequency] = ohos_wow64_get_frequency,
    [get_position] = ohos_wow64_get_position,
    [set_volumes] = ohos_wow64_set_volumes,
    [set_event_handle] = ohos_wow64_set_event_handle,
    [set_sample_rate] = ohos_not_implemented,
    [test_connect] = ohos_wow64_test_connect,
    [is_started] = ohos_is_started,
    [get_prop_value] = ohos_wow64_get_prop_value,
    [midi_get_driver] = ohos_midi_get_driver,
    [midi_init] = ohos_wow64_midi_init,
    [midi_release] = ohos_midi_release,
    [midi_out_message] = ohos_wow64_midi_out_message,
    [midi_in_message] = ohos_wow64_midi_in_message,
    [midi_notify_wait] = ohos_wow64_midi_notify_wait,
    [aux_message] = ohos_wow64_aux_message,
};
