...
/* NOTE: signature must match struct opengl_driver_funcs.p_surface_create of the
 * baseline (Valve 11.0 has an extra leading "BOOL raw").  The WineHua readback
 * driver was written against the 11.10 layout (HWND, format, drawable); keeping
 * that here makes win32u pass (hwnd, raw, format, drawable) into a 3-argument
 * callee, so "drawable" receives the *format* value and the first dereference
 * faults at that small address (observed: SetPixelFormat -> 0xc0000005 with
 * si_addr = 0x6f = pixel format 111). */
BOOL winehua_readback_surface_create(HWND hwnd, BOOL raw, int format, struct opengl_drawable **drawable)
{
    ...
    (void)raw;  /* the 11.10 baseline had no "raw" parameter; readback ignores it */
