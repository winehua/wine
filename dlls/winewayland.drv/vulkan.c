...
/* NOTE: the baseline's struct vulkan_driver_funcs entry is
 *   VkResult (*)(HWND, BOOL raw, const struct vulkan_instance *, VkSurfaceKHR *, struct client_surface **)
 * The WineHua driver was written against the 11.10 layout (no "raw"), and win32u
 * calls it with 5 arguments — keeping the 4-argument form made "instance" receive
 * the raw flag (a small integer) and every vkCreateWin32SurfaceKHR faulted inside
 * the driver (observed as "Exception 0xc0000005 in Unix call" and DXVK reporting
 * featureLevel 0.0). */
                                              ...
                                              VkSurfaceKHR *handle, struct client_surface **client)
{
    ...
    if (winehua_vulkan_present_enabled())
    {
        ...
        *handle = (VkSurfaceKHR)(uintptr_t)(WINEHUA_VULKAN_SURFACE_TAG |
        ...
        /* This private surface is not handed to a host WSI implementation,
         * so nothing else implicitly flushes its Wayland create/subsurface
         * requests before the first private present.  The host presenter
         * keys targets by (Wayland client pid, wl_surface id) and must see
         * those requests before it can attach the SurfaceQueue target. */
