...
/* Register Wine's fault/trap handlers as OHOS musl sigchain special
 * handlers so recoverable translator faults are claimed before DFX dumps
 * and freezes the thread. Keep the two handlers distinct: FEX relies on
 * SIGBUS becoming EXCEPTION_DATATYPE_MISALIGNMENT to emulate unaligned
 * atomics generated by its JIT. */
...
/* TEMP-DIAG(PROF-SAMPLE): arm a SIGPROF sampler when WINEHUA_PROF_SAMPLE is set.
 * Records host PC/LR plus the box64 guest RIP (x27) of the sampled thread so a
 * spinning 32 bit process can be located without a debugger. Diagnostic only:
 * the handler never changes the interrupted context. */
