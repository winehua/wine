...
/* OHOS: the app sandbox seccomp filter kills the process with SIGSYS on
 * userfaultfd(), and kernel_writewatch_init() runs from virtual_init() during
 * every ntdll load — wineboot/wineserver died with signal 31 before printing
 * anything.  Proton's writewatch is Linux-only; keep the upstream fallback. */
#if defined(UFFD_FEATURE_WP_ASYNC) && defined(PM_SCAN_WP_MATCHING) && !defined(__OHOS__)
 ...
 */
#ifdef __OHOS__
/* TEMP-DIAG(DLOPEN-TRACE): the OHOS port hoists unixlib loading onto the host
 * dlopen(); a bogus (typically truncated) path pointer crashes inside the
 * system linker with no Wine-side trace at all.  Log what we hand over. */
void ohos_dlopen_trace( const char *tag, const char *path )
{
    unsigned long p = (unsigned long)path;

    if (!p)
        fprintf( stderr, "[dlopen-trace] %s path=(null)\n", tag );
    else if (p < 0x10000 || p > 0x00007f0000000000UL)
        fprintf( stderr, "[dlopen-trace] %s path=%p SUSPECT-NOT-A-POINTER\n", tag, (void *)p );
    else
        fprintf( stderr, "[dlopen-trace] %s path=%p str='%.200s'\n", tag, (void *)p, path );
}
#endif
    ...
    LIST_FOR_EACH_ENTRY( builtin, &builtin_modules, struct builtin_module, entry )
    {
        ...
        if (builtin->unix_path && !builtin->unix_handle)
        {
            ...
            /* OHOS: dlopen 不能用绝对路径, 只取文件名让系统 linker 搜索 libs/ */
            {
                ...
                ohos_dlopen_trace( "get_builtin_unix_funcs", name );
...
NTSTATUS load_builtin_unixlib( void *module, const char *name )
{
    ...
    server_enter_uninterrupted_section( &virtual_mutex, &sigset );
#ifdef __OHOS__
    /* TEMP-DIAG(PROF-SAMPLE): 装载点原本挂在 dlopen_dll() 上, 但本移植里那条路径
     * 从不被走到 (实测 dlopen_dll trace 0 命中, 而本函数 15 万次) → 采样器永不 arm。
     * 挪到这里; 幂等, 只有 WINEHUA_PROF_SAMPLE 非 0 时才真正干活。 */
    ...
    LIST_FOR_EACH_ENTRY( builtin, &builtin_modules, struct builtin_module, entry )
    {
        ...
        if (!builtin->unix_handle)
        {
            load_steam_overlay(builtin->unix_path);
#ifdef __OHOS__
            ohos_dlopen_trace( "load_builtin_unixlib", builtin->unix_path );
#endif
            builtin->unix_handle = dlopen( builtin->unix_path, RTLD_NOW );
        }
        break;
