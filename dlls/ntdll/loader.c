...
static void __fastcall default_thread_init_func( DWORD unknown, LPTHREAD_START_ROUTINE entry, void *arg )
{
    /* TEMP-DIAG(THREAD-START): a NULL entry here is an immediate jump to 0. */
    MESSAGE( "[thread-start] default_thread_init_func entry=%p arg=%p caller=%p tid=%04x\n",
             entry, arg, __builtin_return_address(0),
             (unsigned int)(ULONG_PTR)NtCurrentTeb()->ClientId.UniqueThread );
    /* TEMP-DIAG(THREAD-START): is the syscall dispatcher slot already injected? */
    MESSAGE( "[thread-start]   syscall_dispatcher=%p\n", __wine_syscall_dispatcher );
    if (!entry)
    {
        MESSAGE( "[thread-start] NULL entry detected - exiting thread instead of jumping to 0\n" );
        RtlExitUserThread( 0 );
    }
...
/* Proton 的 lsteamclient.dll (跳板目标)。只有它被真正装上跳板后, tier0/vstdlib 的
 * 导入才允许被重定向到 ntdll —— Proton 的 ntdll 提供这些导出符号。本树 (WineHua)
 * 没有 lsteamclient.dll, 所以这个变量保持 NULL, 重定向自然不会生效。
 * 2026-09-18: 之前是无条件重定向, 结果原生 steamclient64.dll 对 tier0_s64.dll /
 * vstdlib_s64.dll 的 298 个导入 (含 *数据* 变量 g_pMemAllocSteam) 全被换成 ntdll 的
 * stub; DllMain 解引用那个 stub 立刻访问违例, win64 Steam 客户端因此加载失败
 * (32 位客户端因为模块名不匹配 (steamclient.dll) 不走这条路径, 所以一直正常)。 */
...
static BOOL import_dll( WINE_MODREF *wm, const IMAGE_IMPORT_DESCRIPTOR *descr, LPCWSTR load_path, WINE_MODREF **pwm )
{
    ...
    if (use_lsteamclient() && lsteamclient)
    {
...
{
    /* TEMP-DIAG(EXIT): record deliberate process exits (vs. being killed). */
    MESSAGE( "[exit] RtlExitUserProcess tid=%04x status=0x%08x\n",
...
void loader_init( CONTEXT *context, void **entry )
{
    ...
    if (!imports_fixup_done)
    {
        ...
        /* TEMP-DIAG(THREAD-START) */
        MESSAGE( "[thread-start] pBaseThreadInitThunk=%p (kernel32 base=%p, fallback=%p)\n",
                 pBaseThreadInitThunk, kernel32->ldr.DllBase, default_thread_init_func );
